...
# target_variable = 'Earnings Per Share Reported - Actual'
...
# make data stationary, check for stationarity with ADF
# see how many periods it takes to get the data stationary -> this will be the value for d
...
def check_stationarity(df, column):
    ...
    if p_value > 0.05:
        print("The column is not stationary.")
        
        # Apply differencing and re-run ADF test
        d = 0
        while p_value > 0.05:
            d += 1
            differenced_data = data.diff(periods=d).dropna()
            result = adfuller(differenced_data)
            p_value = result[1]
        
        print(f"Applying {d} order(s) of differencing makes the column stationary.")
    else:
        print("The column is stationary.")


check_stationarity(df, target_variable)
...
# determine the values for parameters p and q by looking at the acf and pacf

your_column_name = target_variable  
your_column = df[your_column_name]

# Plot ACF
plt.figure(figsize=(12, 6))
plot_acf(your_column, lags=20, ax=plt.gca(), title='Autocorrelation Function (ACF)')
plt.show()

# Plot PACF
plt.figure(figsize=(12, 6))
plot_pacf(your_column, lags=20, ax=plt.gca(), title='Partial Autocorrelation Function (PACF)')
plt.show()
